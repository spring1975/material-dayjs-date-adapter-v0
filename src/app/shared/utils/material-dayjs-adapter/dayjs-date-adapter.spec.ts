...
describe("DayjsDateAdapter", () => {
  describe("with defaults", () => {
    ...
    it("should parse string according to given format", () => {
      ...
      expect(
        adapter.parse("1/2/17", ["D/M/YYYY", "MM/DD/YYYY", "D/M/YY"])!.format()
    ...
    it("should compare dates", () => {
      expect(
        adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2017, JAN, 2]))
      ).toBeLessThan(0);
      expect(
        adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2017, FEB, 1]))
      ).toBeLessThan(0);
      expect(
        adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2018, JAN, 1]))
      ).toBeLessThan(0);
      expect(
        adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2017, JAN, 1]))
      ).toBe(0);
      expect(
        adapter.compareDate(dayjs([2018, JAN, 1]), dayjs([2017, JAN, 1]))
      ).toBeGreaterThan(0);
      expect(
        adapter.compareDate(dayjs([2017, FEB, 1]), dayjs([2017, JAN, 1]))
      ).toBeGreaterThan(0);
      expect(
        adapter.compareDate(dayjs([2017, JAN, 2]), dayjs([2017, JAN, 1]))
      ).toBeGreaterThan(0);
    });
    ...
    it("should deserialize dates with the correct locale", () => {
      adapter.setLocale("ja");
      expect(adapter.deserialize("1985-04-12T23:20:50.52Z")!.locale()).toBe(
        "ja"
      );
    ...
    describe("strict mode", () => {
      ...
      it("should detect valid strings according to given format", () => {
        expect(adapter.parse("1/2/2017", "D/M/YYYY")!.format("l")).toEqual(
          dayjs([2017, FEB, 1]).format("l")
        );
      });

      it("should detect invalid strings according to given format", () => {
        expect(adapter.parse("2017-01-01", "MM/DD/YYYY")!.isValid()).toBe(
          false
        );
        expect(adapter.parse("1/2/2017", "MM/DD/YYYY")!.isValid()).toBe(false);
        expect(adapter.parse("Jan 5, 2017", "MMMM D, YYYY")!.isValid()).toBe(
          false
        );
      });
    });

...
import { DayjsDateModule } from './index';
...
fdescribe('DayjsDateAdapter', () => {
    ...
    // setLocale needs to be able to dynamically load locale plugins
    xit('should get month names in a different locale', () => {
    ...
    xit('should get year name in a different locale', () => {
        adapter.setLocale('ar-AE');
        expect(adapter.getYearName(dayjs([2017, JAN, 1]))).toBe('٢٠١٧');
    });
    ...
    it('should create dayJs date', () => {
        expect(adapter.createDate(2017, JAN, 1).format())
            .toEqual(dayjs([2017, JAN, 1]).format());
    });
    ...
    it('should get today\'s date', () => {
        expect(adapter.sameDate(adapter.today(), dayjs()))
            .toBe(true, 'should be equal to today\'s date');
    });

    it('should parse string according to given format', () => {
        expect(adapter.parse('1/2/2017', 'M/D/YYYY')!.format())
            .toEqual(dayjs([2017, JAN, 2]).format());
        expect(adapter.parse('01/02/2017', 'MM/DD/YYYY')!.format())
            .toEqual(dayjs([2017, JAN, 2]).format());
        expect(adapter.parse('1/2/2017', 'D/M/YYYY')!.format())
            .toEqual(dayjs([2017, FEB, 1]).format());
        expect(adapter.parse('1/2/17', ['D/M/YYYY', 'MM/DD/YYYY', 'D/M/YY'])!.format())
            .toEqual(dayjs([2017, FEB, 1]).format());
    });

    it('should parse number', () => {
        const timestamp = new Date().getTime();
        expect(adapter.parse(timestamp)!.format()).toEqual(dayjs(timestamp).format());
    });
    ...
    it('should format date according to given format', () => {
        expect(adapter.format(dayjs([2017, JAN, 2]), 'MM/DD/YYYY')).toEqual('01/02/2017');
        expect(adapter.format(dayjs([2017, JAN, 2]), 'DD/MM/YYYY')).toEqual('02/01/2017');
    });

    // setLocale needs to be able to dynamically load locale plugins
    xit('should format with a different locale', () => {
        expect(adapter.format(dayjs([2017, JAN, 2]), 'll')).toEqual('Jan 2, 2017');
        adapter.setLocale('ja-JP');
        expect(adapter.format(dayjs([2017, JAN, 2]), 'll')).toEqual('2017年1月2日');
    });

    it('should throw when attempting to format invalid date', () => {
        expect(() => adapter.format(dayjs(NaN), 'MM/DD/YYYY'))
            .toThrowError(/DayjsDateAdapter: Cannot format invalid date\./);
    });

    it('should add years', () => {
        expect(adapter.addCalendarYears(dayjs([2017, JAN, 1]), 1).format())
    ...
    it('should respect leap years when adding years', () => {
        expect(adapter.addCalendarYears(dayjs([2016, FEB, 29]), 1).format())
    ...
    it('should add months', () => {
        expect(adapter.addCalendarMonths(dayjs([2017, JAN, 1]), 1).format())
            .toEqual(dayjs([2017, FEB, 1]).format());
        expect(adapter.addCalendarMonths(dayjs([2017, JAN, 1]), -1).format())
    ...
    it('should respect month length differences when adding months', () => {
        expect(adapter.addCalendarMonths(dayjs([2017, JAN, 31]), 1).format())
            .toEqual(dayjs([2017, FEB, 28]).format());
        expect(adapter.addCalendarMonths(dayjs([2017, MAR, 31]), -1).format())
            .toEqual(dayjs([2017, FEB, 28]).format());
    });

    it('should add days', () => {
        expect(adapter.addCalendarDays(dayjs([2017, JAN, 1]), 1).format())
            .toEqual(dayjs([2017, JAN, 2]).format());
        expect(adapter.addCalendarDays(dayjs([2017, JAN, 1]), -1).format())
    ...
    it('should compare dates', () => {
        expect(adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2017, JAN, 2]))).toBeLessThan(0);
        expect(adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2017, FEB, 1]))).toBeLessThan(0);
        expect(adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2018, JAN, 1]))).toBeLessThan(0);
        expect(adapter.compareDate(dayjs([2017, JAN, 1]), dayjs([2017, JAN, 1]))).toBe(0);
        expect(adapter.compareDate(dayjs([2018, JAN, 1]), dayjs([2017, JAN, 1]))).toBeGreaterThan(0);
        expect(adapter.compareDate(dayjs([2017, FEB, 1]), dayjs([2017, JAN, 1]))).toBeGreaterThan(0);
        expect(adapter.compareDate(dayjs([2017, JAN, 2]), dayjs([2017, JAN, 1]))).toBeGreaterThan(0);
    });

    it('should clamp date at lower bound', () => {
        expect(adapter.clampDate(
            dayjs([2017, JAN, 1]), dayjs([2018, JAN, 1]), dayjs([2019, JAN, 1])))
            .toEqual(dayjs([2018, JAN, 1]));
    });

    it('should clamp date at upper bound', () => {
        expect(adapter.clampDate(
            dayjs([2020, JAN, 1]), dayjs([2018, JAN, 1]), dayjs([2019, JAN, 1])))
            .toEqual(dayjs([2019, JAN, 1]));
    });

    it('should clamp date already within bounds', () => {
        expect(adapter.clampDate(
            dayjs([2018, FEB, 1]), dayjs([2018, JAN, 1]), dayjs([2019, JAN, 1])))
    ...
    // setLocale needs to be able to dynamically load locale plugins
    xit('setLocale should not modify global dayjs locale', () => {
    ...
    // setLocale needs to be able to dynamically load locale plugins
    xit('returned dayjs should have correct locale', () => {
        adapter.setLocale('ja-JP');
    ...
    // setLocale needs to be able to dynamically load locale plugins
    xit('should not change locale of dayjs passed as params', () => {
        const date = dayjs();
...
describe('DayjsDateAdapter with MAT_DATE_LOCALE override', () => {
    let adapter: DayjsDateAdapter;
...
describe('DayjsDateAdapter with LOCALE_ID override', () => {
    let adapter: DayjsDateAdapter;
    ...
    it('should take the default locale id from the LOCALE_ID injection token', () => {
        expect(adapter.format(dayjs([2017, JAN, 2]), 'll')).toEqual('2 janv. 2017');
    });
});

describe('DayjsDateAdapter with MAT_DAYJS_DATE_ADAPTER_OPTIONS override', () => {
    let adapter: DayjsDateAdapter;
